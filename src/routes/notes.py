...
from src.schemas import NoteIn, NoteStatusUpdate, NoteOut
...
async def create_note(body: NoteIn, db: Session = Depends(get_db)):
    return await repository_notes.create_note(body, db)


@router.put("/{note_id}", response_model=NoteOut)
async def update_note(body: NoteIn, note_id: int, db: Session = Depends(get_db)):
