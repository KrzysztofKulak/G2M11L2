...
from src.schemas import NoteIn, NoteStatusUpdate
...
async def create_note(body: NoteIn, db: Session) -> Note:
...
async def update_note(note_id: int, body: NoteIn, db: Session) -> Note | None:
    note = db.query(Note).filter(Note.id == note_id).first()
    if note:
        tags = db.query(Tag).filter(Tag.id.in_(body.tags)).all()
